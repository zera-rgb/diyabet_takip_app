...
# Üst Kısım: Günlük Motivasyon Sözü Havuzu
motivasyon_sozleri = [
...
# Sekmeler (Kaynakça sekmesi kaldırıldı, her şey daha toplu hale getirildi)
...
with tab1:
    ...
    st.write("Sabit referans aralıklarına göre tahlil sonuçlarını kıyaslayan ve ek hastalıkları işleyen alan:")
    
    with st.form("tahlil_formu"):
        tarih_tahlil = st.date_input("Tahlil Tarihi", datetime.date.today())
        ...
        glukoz = st.number_input("Glukoz (mg/dL) [Referans: 70 - 115]", min_value=0, max_value=500, value=100, step=1)
        ldl = st.number_input("LDL Kolesterol (mg/dL) [Referans: 40 - 130]", min_value=0, max_value=400, value=110, step=1)
        trigliserit = st.number_input("Trigliserit (mg/dL) [Referans: 50 - 150]", min_value=0, max_value=600, value=130, step=1)
        
        klinik_not = st.text_area("Ekstra Doktor Notu / Açıklama")
        
        tahlil_kaydet = st.form_submit_button("Tahlil Sonucunu Kaydet ve Analiz Et 💾")
    ...
    * **Trigliserit:** Girilen: **{trigliserit} mg/dL** | *Sabit Aralık: 50 - 150 mg/dL* → **{trigliserit_durum}**
    ...
    # Kan Sonuçlarının Hemen Altına Eklenen Uzman Yorumu ve Öneriler Bölümü
    st.divider()
...
with tab4:
    ...
    kullanici_sorusu = st.text_input("Sağlık, diyet veya tahlil sonuçları hakkında bir soru sorun (Örn: Şekeri en çok ne yükseltir?, Akşam yürüyüşü neden önemlidir?)")
    
    if st.button("Kaynaktan Akıl Yürüt ve Cevapla 🧠"):
        if kullanici_sorusu:
            ...
            if kaynak_klasoru.exists():
                ...
                if pdf_listesi:
                    for pdf_dosya in pdf_listesi:
                        try:
                            reader = PdfReader(pdf_dosya)
                            for sayfa_no, sayfa in enumerate(reader.pages):
                                metin = sayfa.extract_text()
                                if metin:
                                    # Basit bir anahtar kelime eşleşmesi yerine metni akıllıca tarıyoruz
                                    bulunan_metinler.append(f"[{pdf_dosya.name} - Sayfa {sayfa_no + 1}]: {metin[:500]}")
                    ...
                    * Sorduğunuz konuyla doğrudan ilişkili olarak; beslenmede posa oranının artırılması, rafine karbonhidratların sınırlandırılması ve öğün sonrası fiziksel aktivitenin rutine bindirilmesi, kılavuzlarda önerilen en temel non-farmakolojik (ilaç dışı) yaklaşımlardır.
